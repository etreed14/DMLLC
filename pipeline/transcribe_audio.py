...
def transcribe_audio(event: Dict[str, Any], context: Any) -> None:
    ...
    if not file_name.startswith(AUDIO_PREFIX):
        print(f"Skipping file outside {AUDIO_PREFIX}: {file_name}")
        return
    ...
    print(f"Starting transcription for {audio_uri}")
    ...
    print(f"Raw transcript saved to gs://{bucket_name}/{raw_json_name}")
    ...
    print(f"Formatted transcript saved to gs://{bucket_name}/{formatted_name}")
