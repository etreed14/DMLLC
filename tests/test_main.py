...
def test_transcribe_endpoint(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(main.requests, 'get', lambda *a, **k: Mock(json=lambda:{'access_token':'tok'}))
    resp_mock = Mock(status_code=200, json=lambda:{'results':[{'alternatives':[{'transcript':'hi'}]}]})
    monkeypatch.setattr(main.requests, 'post', lambda *a, **k: resp_mock)

    client = main.app.test_client()
    rv = client.post('/transcribe', json={'bucket':'b','name':'file.mp3'})
